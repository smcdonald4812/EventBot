...
public class TimerTriggers {
	...
	public void setTriggers(String[] s, MessageReceivedEvent msg) {
		...
		case "yearly":
			for (int i = 0; i < 30; i++) {
				...
				eventDate = repeatEvent(eventDate, repeats);
			}
			break;
		case "monthly":
			for (int i = 0; i < 30; i++) {
	...
	// change UTC to CST
	public String offSetCST(String s) {
		String[] time = s.split(" ");
		String[] offSet = time[1].split(":");
		int temp = Integer.valueOf(offSet[0]);
		temp -= 5;
		offSet[0] = String.valueOf(temp);
		s = time[0] + " " + offSet[0] + ":" + offSet[1];
		return s;
	}
	...
	private String repeatEvent(String s, String re) {
		...
		case "yearly":
			...
			temp++;
			arr[2] = String.valueOf(temp);
			s = "" + arr[0] + "/" + arr[1] + "/" + arr[2] + " " + arr1[1];
			break;
		case "monthly":
			temp = Integer.valueOf(arr[0]);
			temp++;
			arr[0] = String.valueOf(temp);
			s = "" + arr[0] + "/" + arr[1] + "/" + arr[2] + " " + arr1[1];
			break;
		case "weekly":
			temp = Integer.valueOf(arr[1]);
			if (temp < 32) {
				temp += 7;
